...
# -------------------------
# RESET LEASES (ADMIN - SQLITE)
# -------------------------

@app.route("/admin/reset_leases", methods=["POST"])
def reset_leases():

    reclaimed = reclaim_older_than(3)  # 3 days
...
def dashboard():
    ...
    <body>
        ...
        async function resetLeases() {
            ...
            if (!confirm("Reclaim leases older than 3 days?"))
                return;

            let r = await fetch('/admin/reset_leases', {
                method: 'POST'
            });
